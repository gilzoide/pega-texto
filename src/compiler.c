...
static int _pt_compile_literal(pt_bytecode *bytecode, pt_expr *expr) {
	...
	int res = pt_push_byte(bytecode, PT_OP_STRING)
	          && pt_push_byte_array(bytecode, literal_size + 1, (const uint8_t *)expr->data.characters);
...
static int _pt_compile_set(pt_bytecode *bytecode, pt_expr *expr) {
	...
	int res = pt_push_byte(bytecode, PT_OP_SET)
	          && pt_push_byte_array(bytecode, set_size + 1, (const uint8_t *)expr->data.characters);
	return res ? 2 : PT_COMPILE_MEMORY_ERROR;
}
...
found_name: {}
	int result = pt_push_bytes(bytecode, 2, PT_OP_CALL, N);
...
static int _pt_compile_quantifier(pt_bytecode *bytecode, pt_grammar *g, pt_expr *expr, pt_rule_compile_state compile_state[]) {
	int N = expr->N;
	if((N < -1 || N > 0) && !pt_push_byte(bytecode, PT_OP_RESET_QC)) return PT_COMPILE_MEMORY_ERROR; 
	...
	int after_label_offset = 0;
	if(N < 0) {  // at most N
		if(!pt_push_byte(bytecode, PT_OP_ACCEPT)) return PT_COMPILE_MEMORY_ERROR;
		// TODO: N < -1
	}
	else {  // at least N
		int current_size = bytecode->chunk.size;
		int should_emit_qc_instructions = N > 0;
		if(!pt_reserve_bytes(bytecode, 4 + (3 * should_emit_qc_instructions))) return PT_COMPILE_MEMORY_ERROR;
		uint8_t *instr_ptr = pt_byte_at(bytecode, current_size);
		if(should_emit_qc_instructions) {
			*instr_ptr = PT_OP_INC_QC;
			instr_ptr++;
		}
		instr_ptr[0] = PT_OP_SAVE_SP;
		instr_ptr[1] = PT_OP_JUMP_ABSOLUTE;
		*((int16_t *)(instr_ptr + 2)) = (int16_t)previous_size + 3;
		if(should_emit_qc_instructions){
			instr_ptr[4] = PT_OP_FAIL_QC_LESS_THAN;
			instr_ptr[5] = (uint8_t)N;
			after_label_offset = -2;
		}
	}

	int current_size = bytecode->chunk.size;
	uint8_t *jmp_op_ptr = pt_byte_at(bytecode, previous_size);
	jmp_op_ptr[0] = PT_OP_SET_ADDRESS_FAIL;
	*((uint16_t *)(jmp_op_ptr + 1)) = (uint16_t)current_size + after_label_offset;
...
static int _pt_compile_choice(pt_bytecode *bytecode, pt_grammar *g, pt_expr *expr, pt_rule_compile_state compile_state[]) {
	int N = expr->N;
	if(N > 0 && expr->data.es == NULL) return PT_COMPILE_NULL_POINTER;
	int i, res = 0, previous_size, initial_size = bytecode->chunk.size;
	if(!pt_reserve_bytes(bytecode, 4)) return PT_COMPILE_MEMORY_ERROR;
	for(i = 0; res >= 0 && i < N; i++) {
		previous_size = bytecode->chunk.size;
		if(!pt_reserve_bytes(bytecode, 3)) return PT_COMPILE_MEMORY_ERROR;
		res = pt_compile_expr(bytecode, g, expr->data.es[i], compile_state);
		if(res >= 0) {
			if(!pt_push_byte(bytecode, PT_OP_POP_ACCEPT)) return PT_COMPILE_MEMORY_ERROR;
			int current_size = bytecode->chunk.size;
			uint8_t *instr_ptr = pt_byte_at(bytecode, previous_size);
			instr_ptr[0] = PT_OP_SET_ADDRESS_FAIL;
			*((uint16_t *)(instr_ptr + 1)) = (uint16_t)current_size;
		}
	}
	if(!pt_push_byte(bytecode, PT_OP_POP_FAIL)) return PT_COMPILE_MEMORY_ERROR;
	uint8_t *instr_ptr = pt_byte_at(bytecode, initial_size);
	instr_ptr[0] = PT_OP_PUSH;
	instr_ptr[1] = PT_OP_SET_ADDRESS_ACCEPT;
	*((uint16_t *)(instr_ptr + 2)) = (uint16_t)bytecode->chunk.size;
	return res;
}
...
enum pt_compile_status pt_compile_grammar(pt_bytecode *bytecode, pt_grammar *g) {
	...
	for(i = 0; result >= 0 && i < N; i++) {
		...
		*pt_list_at(&bytecode->rule_addresses, i, uint16_t) = bytecode->chunk.size;
		pt_expr *expr = g->es[i];
		result = pt_compile_expr(bytecode, g, expr, compile_state);
		if(result >= 0) result = pt_push_byte(bytecode, PT_OP_POP_ACCEPT);
	}
	if(result >= 0) result = pt_push_byte(bytecode, PT_OP_POP_FAIL); // common fail operation for calls
